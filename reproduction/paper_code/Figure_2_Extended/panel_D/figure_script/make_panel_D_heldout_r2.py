...
def main() -> None:
    ...
    summary = frame.groupby(["axis", "n_train_villi"])["r2"].agg(mean="mean", sem="sem", runs="count").reset_index()
    out = args.output_dir.expanduser().resolve()
    stem, table, provenance = out / "panel_D_heldout_villus_r2", out / "panel_D_heldout_villus_r2_summary.tsv", out / "panel_D_heldout_villus_r2_provenance.json"
    ...
    for coordinate in COLORS:
        ...
        for count, points in frame.loc[frame.axis.eq(coordinate)].groupby('n_train_villi'):
            ...
            axis.scatter(count + np.linspace(-.55, .55, len(points)), points.r2, s=8, alpha=.30, color=COLORS[coordinate], linewidths=0)
