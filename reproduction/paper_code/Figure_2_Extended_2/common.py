...
PREPARED_MANIFEST_SHA256 = "ef18bf9315c658c5b2fa8bfea1e5bc471dbde39d31af37eb4b5c978d2d6bebe6"
...
def load_prepared_manifest(path: str | Path) -> tuple[Path, dict[str, object]]:
    """Pin the release; each renderer verifies only its selected input files."""
    manifest_path = require_file(path, PREPARED_MANIFEST_SHA256)
...
def save_pair(figure, stem: Path, dpi: int = 300) -> list[Path]:
    pdf = stem.with_suffix(".pdf")
    png = stem.with_suffix(".png")
    figure.savefig(pdf, bbox_inches="tight", dpi=dpi)
